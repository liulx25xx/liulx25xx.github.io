          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              <h3 className=" text-base lg:text-lg font-medium font-grotesk">
                ...
                {companiesInfo[tab].companyUrl ? (
                  <a
                    href={companiesInfo[tab].companyUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-secondary-color-3 hover:underline ml-1"
                  >
                    {companiesInfo[tab].company}
                  </a>
                ) : (
                  <span className="text-secondary-color-3 ml-1">
                    {companiesInfo[tab].company}
                  </span>
                )}
              </h3>
              ...
              {companiesInfo[tab].roles.map((role) => (
                <div key={role} className="flex items-start mt-2">
                  ...
                  <p className="text-base pt-1">{role}</p>
                </div>
              ))}
              
              {/* Optional Company Image */}
              {(companiesInfo[tab] as any).image && (
                <div className="mt-4 flex justify-center lg:justify-start w-full">
                  <div className="relative w-full max-w-sm h-32 sm:h-40 rounded-xl overflow-hidden bg-transparent">
                    <Image
                      src={(companiesInfo[tab] as any).image}
                      alt={`${companiesInfo[tab].company} banner`}
                      fill
